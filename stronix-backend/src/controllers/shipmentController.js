...
export const dispachShipment = async(req, res) => {
    try {
        ...
        const order = await Order.findById(shipment.order).populate("items");
        if (!order) {
        ...
        for (const orderItem of order.items) {
            ...
            if (!inventory) {
                return res.status(400).json({ message: "Insufficient reserved stock for dispatch" });
            }
            ...
            movements.push({
                ...
                availableDelta: 0,
                reservedDelta: -qty,
                reference: { entityType: "Shipment", entityId: String(shipment._id), reason: "DISPATCHED_FROM_WAREHOUSE" },
        ...
        await AuditLog.create({
            user: req.user?._id,
            action: "DISPATCHED_FROM_WAREHOUSE",
            entity: "Shipment",
            entityId: shipment._id
        });
